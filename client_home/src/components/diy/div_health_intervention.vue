<template>
	<div class="diy_details_box diy_div_health_intervention">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "health_intervention_id",
					source_id: this.obj["health_intervention_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/health_intervention/set?health_intervention_id=' + _this.obj["health_intervention_id"], {
								praise_len
							}, (res) => {
								if(res.result){
