<template>
	<div class="diy_details_box diy_div_member_files">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "member_files_id",
					source_id: this.obj["member_files_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/member_files/set?member_files_id=' + _this.obj["member_files_id"], {
								praise_len
							}, (res) => {
								if(res.result){
