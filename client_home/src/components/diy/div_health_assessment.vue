<template>
	<div class="diy_details_box diy_div_health_assessment">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "health_assessment_id",
					source_id: this.obj["health_assessment_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/health_assessment/set?health_assessment_id=' + _this.obj["health_assessment_id"], {
								praise_len
							}, (res) => {
								if(res.result){
