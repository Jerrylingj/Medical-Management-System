<template>
	<div class="diy_details page_physical_examination_package" id="physical_examination_package_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_physical_examination_package'>
							<div_physical_examination_package :obj="obj"></div_physical_examination_package>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_physical_examination_package from "@/components/diy/div_physical_examination_package.vue";
	...
	export default {
		...
		components:{
			...
			bar_title,
			form_editor,
			div_physical_examination_package
		},
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "physical_examination_package_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/physical_examination_package/set?physical_examination_package_id=' + obj["physical_examination_package_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "physical_examination_package",
							source_field: "physical_examination_package_id",
							source_id: this.obj.physical_examination_package_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.physical_examination_package_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "physical_examination_package",
					source_field: "physical_examination_package_id",
					source_id: obj["physical_examination_package_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/physical_examination_package/set?physical_examination_package_id=' + _this.obj["physical_examination_package_id"], {
								praise_len
							}, (res) => {
								if(res.result){
