		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/physical_examination_package/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_physical_examination_package = !show_list_physical_examination_package">
		                        <div class="switch_box">
		                            <span v-if="show_list_physical_examination_package"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--体检套餐推荐列表组件 -->
		                <list_physical_examination_package v-if="show_list_physical_examination_package" :list="list_physical_examination_package" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_physical_examination_package.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_physical_examination_package" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_physical_examination_package" :key="i" @click="to_details('physical_examination_package',o,'physical_examination_package_id')">
		                            <td v-for="(oj,n) in fields_physical_examination_package">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	...
	import list_physical_examination_package from "@/components/diy/list_physical_examination_package.vue";
	...
	export default {
		mixins: [mixin],
		components: {
			list_physical_examination_package,
			bar_title,
			list_article,
			swiper_img,
			swiper_notice,
		...
		data() {
			return {
				...
				show_list_physical_examination_package: true,
				list_physical_examination_package: [],
				vm_arr_physical_examination_package: [
					"physical_examination_package_id",
					"package_number",
					"package_name",
					"cover",
					"price",
				...
				fields_physical_examination_package: [
					{
						key: "package_number",
						label: "套餐编号",
						type: "文本"
		...
		methods: {
			...
			// 获取体检套餐列表
			get_physical_examination_package() {
				let url = "~/api/physical_examination_package/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_physical_examination_package = json.result.list;
					}
				})
			},
		...
		mounted() {
			this.get_physical_examination_package();
			this.get_menu();
			this.get_slides();
			this.get_article();
			this.get_link();
		...
		computed: {
		    list_table_physical_examination_package() {
		        var list = this.list_physical_examination_package;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_physical_examination_package.map((o) => {
		                // 第二个中括号是对象的属性
