<template>
	<el-main class="bg">
			...
			<el-row>
				...
				<el-col :xs="24" :sm="24" :lg="8">
					<el-form-item label="套餐编号">
						<el-input v-model="query.package_number"></el-input>
					</el-form-item>
				</el-col>
				<el-col :xs="24" :sm="24" :lg="8">
				...
				<el-col :xs="24" :sm="10" :lg="8">
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/physical_examination_appointment/table','add') || $check_action('/physical_examination_appointment/view','add')" class="el-button el-button--default el-button--primary" to="./view?">添加
						</router-link>
						<el-button v-if="user_group == '管理员' || $check_action('/physical_examination_appointment/table','del') || $check_action('/physical_examination_appointment/view','del')" class="" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			...
			<el-table-column prop="package_number" label="套餐编号"
				v-if="user_group == '管理员' || $check_field('get','package_number')" min-width="200">
			</el-table-column>
			...
			<el-table-column prop="total_price" label="总价格"
			...
			<el-table-column prop="appointment_date_of_physical_examination" label="预约体检日期"
				v-if="user_group == '管理员' || $check_field('get','appointment_date_of_physical_examination')" min-width="200">
                <template slot-scope="scope">
                	{{ $toTime(scope.row["appointment_date_of_physical_examination"],"yyyy-MM-dd") }}
                </template>
			</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/physical_examination_appointment/table','set') || $check_action('/physical_examination_appointment/view','set') || $check_action('/physical_examination_appointment/view','get') || $check_action('/体检结果/table','add') || $check_action('/体检结果/view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/physical_examination_appointment/table','set') || $check_action('/physical_examination_appointment/view','set') || $check_action('/physical_examination_appointment/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
					<!--跨表按钮-->
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/physical_examination_results/view')" v-if="user_group == '管理员' || $check_action('/physical_examination_results/table','add') || $check_action('/physical_examination_results/view','add')">
						<span>体检结果</span>
					</el-button>
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && (user_group == '管理员' || $check_pay('/physical_examination_appointment/table'))">
						<span>支付</span>
					</el-button>
				</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/physical_examination_appointment/get_list?like=0",
				url_del: "~/api/physical_examination_appointment/del?",
		...
		methods: {
			...
			openPayModal(v){
				this.pay_obj.id = v.physical_examination_appointment_id
				this.payModalVisible = true
			},
			submitPay(){
				let _this = this;
				let url = this.$toUrl(this.query, "~/api/physical_examination_appointment/set?physical_examination_appointment_id="+this.pay_obj.id);
